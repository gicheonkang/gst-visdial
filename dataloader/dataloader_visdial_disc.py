...
class VisdialDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._mode == 'vd_train':
            # train split, train mode, disc model
            utterances = []
            utterances_random = []
            negative_samples_option_indices = []
            ...
            # -------------------------------------------------------------------
            # Structuring positive / negative dialog data
            # -------------------------------------------------------------------
            for rnd,utterance in enumerate(dialog['dialog']):
                ...
                # -------------------------------------------------------------------
                # Randomly selecting negative samples while considering MAX_SEQ_LEN
                # -------------------------------------------------------------------
                for _ in range(self.params["num_negative_samples"]):
                    ...
                    while len(all_inds):
                        ...
                        if(MAX_SEQ_LEN >= (tot_len + len(tokenized_random_utterance) + 1)):
                            negative_samples_indices.append(option_ind)
                            break
                        else:
                ...
                negative_samples_option_indices.append(negative_samples_indices)
            # removing the caption in the beginning
            utterances = utterances[1:]
            utterances_random = utterances_random[1:]
            assert len(utterances) == len(utterances_random) == len(negative_samples_option_indices) == 10
            ...
            for j,context in enumerate(utterances):
                ...
                tokens, segments, sep_indices, mask, _ = encode_input(
                    context, 
                    start_segment, 
                    self.CLS,
                    self.SEP, 
                ...
                next_labels_all.append(torch.tensor([1.0, 0.0]).unsqueeze(0)) # ground-truth answer is assigned 1.0 point
                segments_all.append(segments)
                hist_len_all.append(torch.LongTensor([len(context)-1]))
                negative_samples = utterances_random[j]

                for nsi,context_random in enumerate(negative_samples):
                    context_random, start_segment = pruneRounds(context_random, self.params['visdial_tot_rounds'])
                    tokens_random, segments_random, sep_indices_random, mask_random, _ = encode_input(
                        context_random, 
                        start_segment, 
                        self.CLS, 
                        self.SEP, 
                    ...
                    negative_sample_indice = negative_samples_option_indices[j][nsi]
                    if self.params['train_dense']:
                        # negative samples could have partial points -> retrieve non-zero scores from the pseudo dense labels
                        score = self.visdial_data_train_dense[index]['scores'][j][negative_sample_indice]
                        next_labels_all.append(torch.tensor([score, 1.0-score]).unsqueeze(0))
                    else:
                        # use a single ground-truth label by default
                        next_labels_all.append(torch.tensor([0.0, 1.0]).unsqueeze(0))
            ...
            # val split, eval mode, disc model
            ...
            # test split, eval mode, disc model
