...
def sequence_mask(sequence_length, params, max_len=None):
    if max_len is None:
    ...
    if sequence_length.is_cuda:
        seq_range_expand = seq_range_expand.to(params["gpu_ids"][0])
    seq_length_expand = (sequence_length.unsqueeze(1)
                         .expand_as(seq_range_expand))
    return seq_range_expand < seq_length_expand

def batch_iter(dataloader, params, start_epoch=0):
    for epochId in range(start_epoch, params['num_epochs']):
...
def encode_input(utterances, start_segment, CLS, SEP, MASK, PAD, max_seq_len=256, max_sep_len=25, mask_prob=0.15):    
    ...
    mask = masked_tokens[0,:]==1 # 1:mask, 0:non-mask
    masked_tokens[0,mask] = tokens[0,mask]
    tokens[0,mask] = MASK
    segment_id_list = list2tensorpad(segment_id_list,max_seq_len)
    att_mask = (tokens!=0).float()

    return tokens, segment_id_list, list2tensorpad(sep_token_indices,max_sep_len), masked_tokens, att_mask
