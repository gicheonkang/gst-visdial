...
class BertEmbeddingsDialog(nn.Module):
    def __init__(self, config):
        ...
        self.pe = pe.to(torch.device("cuda", config.__dict__['cur_device']))
    ...
    def forward(self, input_ids, sep_indices=None, token_type_ids=None):
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        if 'train' in self.config.mode:
            if 'enc_dec' in self.config.model_arch:
                ...
                logprobs = F.log_softmax(seq_relationship_score.view(-1, 2))
                next_sentence_loss = -(next_sentence_label * logprobs).sum() / next_sentence_label.shape[0]
